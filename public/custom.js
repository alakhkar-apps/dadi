...
(function () {
  ...
  const STYLES = `
    ...
    /* ── Push login form to left half ── */
    #dadi-right-panel ~ * { /* nothing — form already lives left */ }

    /* ── RESPONSIVE ── */

    /* Tablet (768–1023px): narrower panel, smaller image, hide card & pills */
    @media (max-width: 1023px) {
      #dadi-right-panel {
        width: 42%;
      }
      #dadi-right-panel img {
        height: 70vh; max-height: 480px;
      }
      #dadi-right-panel .drp-halo {
    ...
    /* Small tablet (600–767px): strip further, image only */
    ...
    /* Mobile (< 600px): hide panel so login form gets full width */
  ...
  function isLoginPage() {
    return !!(
      ...
      document.querySelector('form') && window.location.pathname.includes('login') ||
      document.querySelector('[class*="LoginPage"]') ||
      document.querySelector('[class*="AuthPage"]') ||
      document.querySelector('[class*="login"]')
    );
  }

  /* ── Inject the right panel ── */
  ...
  /* ── Remove panel when leaving login page ── */
  ...
  /* ── Inject Skip Login button into Chainlit's form ── */
  function injectSkipButton() {
    ...
    note.textContent = "Dadi won't be able to save your chat history.";
    ...
    btn.onclick = async () => {
      ...
      try {
        // POST directly to Chainlit's OAuth2 login endpoint
        ...
        if (resp.ok) {
          ...
          console.error('[Dadi] Guest login failed:', resp.status, await resp.text());
          btn.textContent = 'Skip Login →';
          btn.disabled = false;
        }
      } catch (e) {
        console.error('[Dadi] Guest login error:', e);
        btn.textContent = 'Skip Login →';
        btn.disabled = false;
      }
    };
  ...
  /* ── Watch for React DOM changes (Chainlit SPA) ── */
